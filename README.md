...
- 🧭 **Coach Agent（route）**——问卷摸清你的水平 → 生成分阶段可检验的学习路线 → 进入陪练模式，全程对话驱动，自主调用工具推进学习
...
- 🧠 **记忆系统**——每次提问自动检索已有笔记注入回答；记忆冲突解决、上下文压缩后自动整理事实、偏好、对话摘要

- 🪄 **自动沉淀**——陪练中学到的知识自动进库：达到阈值触发后台线程，提取知识新增入库，对话零阻塞，知识库不臃肿；
...
- **有界上下文**：上下文由「自动压缩」维持在最近 **5 轮（约 16～26 条）** 与画像 / 路线 / 三舱记忆固定注入；

- **自动压缩**：消息超过 **40 条**时触发，被切掉的旧消息经「三舱记忆整理」（见 [记忆系统](#记忆系统)）吸收后丢弃；
...
| 错误分档   | 瞬时错误（连接 / 超时 / 429 / 5xx）退避重试且总耗时封顶；鉴权 / 模型名等确定性错误立即失败并说明原因 |
| 超时分级   | 对话通道 45s / 报告通道 120s，均为实测定值（换模型需重测）；报告生成单次不重试 |
| 调用埋点   | 每次 LLM 调用输出一行 JSON 到 stderr：耗时 / `finish_reason` / token 数 / 重试与降级路径 / 调用点。默认开启；**只记元数据，提示词与正文不进日志** |
